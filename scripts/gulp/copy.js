...
function copyComponentsBoilerplate() {
  return gulp
    ...
    .pipe(
      gulpForeach(function(stream, file) {
        const component = file.path.split('/').pop();
        gulp
          .src('./scripts/templates/component-boilerplate/*')
  ...
  return gulp
    ...
    .pipe(
      gulpForeach(function(stream, file) {
        ...
        gulp.src(src).pipe(gulp.dest('./tmp/' + component));
        return stream;
      })
    );
}
...
function copyComponentsManifest() {
  return gulp
    .src('./src/' + (parseComponentName || '*') + '/package.json')
    .pipe(
      gulpData(file => {
